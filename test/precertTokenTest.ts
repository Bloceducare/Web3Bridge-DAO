...
describe("PreCertificateToken", function () {
  async function deploysPreCertificateToken() {
    ...
    // deploying Certificate contract
    const PrCertificate = await ethers.getContractFactory(
      "PreCertificateToken"
    );
    const precertificate = await PrCertificate.deploy(
      owner.address,
      vault10.address,
      vault5.address,
      _vault5_,
  ...
  describe("Testing PreCertificateToken Functions", function () {
    it("should verify the whitelisted addresses and payFee", async function () {
      ...
      console.log(
        "The balance of the Signer is SignerBalance: ",
        signerBalance
      );
      ...
      const checkComplete = await precertificate.checkCompleted(
        "0x42E0e153135863c41301aDaaC83Cf3e5248aDF9c"
      );
