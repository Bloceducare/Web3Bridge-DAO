...
describe("DAOtoken", function () {
  ...
  describe("testing", function () {
    it("returns the amount to mint to each person", async function () {
      const {otherAccount,daotoken,certificate} = await loadFixture(deploysDaotoken);
    ...
    it("set minting to true", async function () {
      const {otherAccount,daotoken,certificate, add2} = await loadFixture(deploysDaotoken);
      await certificate.safeMint(otherAccount.address,"1");
      await daotoken.setMintAmountPerPerson("20");
      expect (await daotoken.getMintperPerson()).to.equal("20");
    ...
    it("set minting to true and mints to owner", async function () {
      const {otherAccount,daotoken,certificate, add2} = await loadFixture(deploysDaotoken);
      await certificate.safeMint(otherAccount.address,"1");
      await daotoken.setMintAmountPerPerson("20");
      expect (await daotoken.getMintperPerson()).to.equal("20");
    ...
    it("set minting to true and checks the amount of token minted", async function () {
      const {otherAccount,daotoken,certificate, add2} = await loadFixture(deploysDaotoken);
      await certificate.safeMint(otherAccount.address,"1");
      await daotoken.setMintAmountPerPerson("20");
      expect (await daotoken.getMintperPerson()).to.equal("20");
